...
void CResFile::OpenResource( LPCSTR lpszResName )
{
	...
	//DecryptionNew( byEncryptionKey, (BYTE*)pCryptHeader, nFileHeaderSize );
	//pHeader = pCryptHeader;
	...
	memcpy( strVersion, &pHeader[ nHeaderPosition ], 7 );
	nHeaderPosition += 7;

	if( strcmpi( this->strVersion, strVersion ) )
	{
		safe_delete_array(pHeader);
		return;
	}

	//DecryptionNew( byEncryptionKey, (BYTE*)pHeader, nHeaderPosition, sizeof( short ) );
	memcpy( &nFileNumber, &pHeader[ nHeaderPosition ], sizeof( short ) );
	nHeaderPosition += sizeof( short );

	time_t time_;
	for( int i = 0; i < nFileNumber; i++ )
	{
		//DecryptionNew( byEncryptionKey, (BYTE*)pHeader, nHeaderPosition, sizeof( short ) );
		memcpy( &nFileNameLength, &pHeader[ nHeaderPosition ], sizeof( short ) );
		nHeaderPosition += sizeof( short );
		//DecryptionNew( byEncryptionKey, (BYTE*)pHeader, nHeaderPosition, nFileNameLength + ( sizeof( int ) * 3 ));
		memcpy( szFileName, &pHeader[ nHeaderPosition ], nFileNameLength );
		nHeaderPosition += nFileNameLength;
		memcpy( &nFileSize, &pHeader[ nHeaderPosition ], sizeof( int ) );
		nHeaderPosition += sizeof( int );
		memcpy( &time_, &pHeader[ nHeaderPosition ], sizeof( int ) );
		nHeaderPosition += sizeof( int );
		memcpy( &nFilePosition, &pHeader[ nHeaderPosition ], sizeof( int ) );
		nHeaderPosition += sizeof( int );
		...
		lpRes.time_ = time_;
		strcpy( lpRes.szFileName, szFileName );

		m_vecResourceFiles.push_back(lpRes);
...
void CResFile::Save()
{
	...
	char szDir[260];
	_splitpath( m_szFileName, NULL, szDir, NULL, NULL );
	CString strCommand;
	strCommand.Format( "mkdir \"%s\" 1>nul 2>nul", szDir );
	system( strCommand );
	CStdioFile file;
	if( file.Open( m_szFileName, CFile::modeWrite | CFile::modeCreate | CFile::typeBinary ) == TRUE )
	{
		int nOffset = 0;
		nOffset += sizeof(BYTE);
		file.Write(&m_byEncryptionKey, sizeof(BYTE));
		nOffset += sizeof(bool);
		file.Write(&m_bEncryption, sizeof(bool));
		nOffset += sizeof(int);

		int nHeaderSize = 0;
		short nFileCount = m_vecResourceFiles.size();

		nHeaderSize += 7; // version
		nHeaderSize += sizeof(short); // fileNumber

		for( int i = 0; i<  m_vecResourceFiles.size();i++ )
		{
			nHeaderSize += sizeof(short) + 3*sizeof(int) + strlen( m_vecResourceFiles[i].szFileName );
		}

		file.Write( &nHeaderSize, sizeof(int) );
		nOffset += nHeaderSize;

		byte* byHeader = new byte[nHeaderSize];
		int nHelpOffset = 0;

		memcpy( &byHeader[nHelpOffset], strVersion, 7 );
		nHelpOffset += 7;

		memcpy( &byHeader[nHelpOffset], &nFileCount, sizeof(short) );
		nHelpOffset += sizeof(short);

		for( int i = 0; i < m_vecResourceFiles.size(); i++ )
		{
			short nNameLength = strlen( m_vecResourceFiles[i].szFileName );
			memcpy( &byHeader[nHelpOffset], &nNameLength, sizeof(short) );
			nHelpOffset += sizeof(short);
			memcpy( &byHeader[nHelpOffset], m_vecResourceFiles[i].szFileName, nNameLength );
			nHelpOffset += nNameLength;
			memcpy( &byHeader[nHelpOffset], &m_vecResourceFiles[i].dwFileSize, sizeof(int) );
			nHelpOffset += sizeof(int);
			memcpy( &byHeader[nHelpOffset], &m_vecResourceFiles[i].time_, sizeof(int) );
			nHelpOffset += sizeof(int);
			m_vecResourceFiles[i].dwOffset = nOffset;
			nOffset += m_vecResourceFiles[i].dwFileSize;
			memcpy( &byHeader[nHelpOffset], &m_vecResourceFiles[i].dwOffset, sizeof(int) );
			nHelpOffset += sizeof(int);
		}

		//for( int i = 0; i < nHeaderSize; i++ )
		{
			EncryptionNew(m_byEncryptionKey, byHeader, 0, nHeaderSize );
		}

		file.Write( byHeader, nHeaderSize );
		delete[] byHeader;
		file.Flush();
		for( int i = 0; i < m_vecResourceFiles.size(); i++ )
		{
			CString strfilepath;
			strfilepath.Format( "%s%s", m_vecResourceFiles[i].szPath, m_vecResourceFiles[i].szFileName );
			CFile addfile;
			if( addfile.Open( strfilepath, CFile::modeRead ) )
			{
				byte* byFile = new byte[m_vecResourceFiles[i].dwFileSize];
				addfile.Read(byFile, m_vecResourceFiles[i].dwFileSize );
				if( m_bEncryption )
				{
					//for( int k = 0; k < m_vecResourceFiles[i].dwFileSize; k++ )
					{
						EncryptionNew( m_byEncryptionKey, byFile, 0, m_vecResourceFiles[i].dwFileSize );
					}
				}
				file.Write( byFile, m_vecResourceFiles[i].dwFileSize );
				file.Flush();
				delete[] byFile;
			}
		}
...
BOOL CResFile::Open( LPCTSTR lpszFileName, TCHAR *mode )
{
	...
	GetCurrentDirectoryA( sizeof( szSerchPath ), szSerchPath );
	TCHAR drive[_MAX_DRIVE], dir[_MAX_DIR], name[ _MAX_FNAME ], ext[_MAX_EXT];
	...
	else if( CFileIO::Open( lpszFileName, mode ) == TRUE )
	{
#ifdef __SECURITY_0628
#ifndef _DEBUG
#ifdef __MAINSERVER
